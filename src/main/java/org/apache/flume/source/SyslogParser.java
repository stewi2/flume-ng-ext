    ...
    if(rfc5424) {
    	...
    	if(msg.charAt(curPos)!='-') {
    		while(curPos<msg.length() && msg.charAt(curPos)=='[') {
    			curPos+=1; // Skip the '['
    			nextSpace = msg.indexOf(' ', curPos);
    			if(nextSpace==-1) throw new IllegalArgumentException(
    					"bad syslog format (malformed structured data)");

    			String sdid = msg.substring(curPos,nextSpace);
    			int endPos = msg.indexOf(']', curPos);
    			if(endPos==-1) throw new IllegalArgumentException(
    					"bad syslog format (malformed structured data)");
    			...
    			while(curPos<endPos) {
    				nextSpace = msg.indexOf(' ', curPos);
    				if(nextSpace>endPos || nextSpace==-1) nextSpace=endPos;
